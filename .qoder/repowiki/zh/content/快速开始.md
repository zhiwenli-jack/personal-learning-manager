...
## 更新摘要
**所做更改**
- 新增全局启动脚本章节，介绍 start_all.sh 的一键启动功能
- 更新启动流程章节，增加全局启动脚本的使用说明
- 新增一键启动的优势和使用场景说明
- 更新故障排除指南，增加全局启动脚本相关的故障排查
...
## 架构总览
系统采用前后端分离架构，前端通过代理访问后端 API。后端负责业务逻辑、数据库操作与 AI 能力调用；前端负责用户交互与展示。新增的全局启动脚本提供了一键启动所有服务的能力。
...
## 结论
通过本指南，您可以完成环境准备、配置与依赖安装、数据库初始化以及前后端的启动与基本使用。系统提供了资料上传、知识点提炼与题目生成的核心能力，适合个人学习与教学辅助场景。新增的全局启动脚本进一步简化了开发环境的启动流程，提升了开发效率。如需扩展，可在现有 API 与数据库模型基础上继续开发。
