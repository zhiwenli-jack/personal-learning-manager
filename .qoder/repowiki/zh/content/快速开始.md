...
- [start_all.sh](file://start_all.sh)
...
## 更新摘要
**所做更改**
- 更新后端启动流程章节，反映从 subprocess.run 到 subprocess.Popen 的重大改进
- 新增优雅关闭和信号处理机制的详细说明
- 更新进程管理和资源清理的相关内容
- 增强故障排除指南，包含新的信号处理相关问题排查
...
subgraph "启动脚本"
SA["start_all.sh<br/>全局一键启动"]
SB["start_backend.py<br/>后端启动已改进"]
...
- [start_all.sh](file://start_all.sh#L1-L14)
- [start_backend.py](file://start_backend.py#L1-L36)
...
- 后端框架与运行
  ...
  - **已重大改进**：使用 subprocess.Popen 替代 subprocess.run，实现进程控制与优雅关闭。
- 前端框架与运行
...
- 一键启动功能
  - 新增 start_all.sh 全局启动脚本，支持一键启动后端和前端服务。
...
## 架构总览
系统采用前后端分离架构，前端通过代理访问后端 API。后端负责业务逻辑、数据库操作与 AI 能力调用；前端负责用户交互与展示。新增的全局启动脚本提供了一键启动所有服务的能力。**后端启动脚本现已实现优雅关闭机制，确保资源正确清理**。
...
SA["全局启动脚本<br/>start_all.sh"] --> VP
SA --> AP
SB["后端启动脚本<br/>start_backend.py已改进"] --> AP
SB --> SIG["信号处理<br/>SIGTERM/SIGINT"]
SIG --> CLEAN["优雅关闭<br/>资源清理"]
...
- [backend/.env.example](file://backend/.env.example#L1-L14)
- [start_all.sh](file://start_all.sh#L1-L14)
- [start_backend.py](file://start_backend.py#L20-L29)

## 详细组件分析

### 全局启动脚本（一键启动）
新增的 start_all.sh 提供了一键启动整个开发环境的功能，简化了开发流程：
...
SA->>SB : 启动后端服务后台
SA->>SF : 启动前端服务后台
SB->>UV : 启动 Uvicorn 主机 : 0.0.0.0 : 8000
SF->>VITE : 启动 Vite 开发服务器
SA->>SA : 等待所有后台进程
SA-->>Dev : 显示启动完成
```
...
### 后端启动流程（开发模式）- **已重大改进**
**更新**：后端启动脚本已从 subprocess.run 改进为 subprocess.Popen，增加了优雅关闭机制

- **进程控制改进**
  - 使用 subprocess.Popen 替代 subprocess.run，实现对子进程的完全控制
  - 通过 process.terminate() 和 process.wait() 实现优雅关闭
  - 支持后台运行和前台调试两种模式

- **信号处理机制**
  - 注册 SIGTERM 信号处理器（进程终止信号）
  - 注册 SIGINT 信号处理器（Ctrl+C 中断信号）
  - 收到信号时执行清理程序并退出
...
Dev->>SB : 运行后端启动脚本
SB->>UV : 启动 Uvicorn 主机 : 0.0.0.0, 端口 : 8000
UV->>APP : 加载 app/main.py
APP->>DB : 启动事件创建表与上传目录
APP-->>Dev : 暴露 /api/* 接口
SIG->>SB : 监听 SIGTERM/SIGINT
SB->>UV : 优雅关闭子进程
UV-->>SB : 进程终止
SB-->>Dev : 清理完成
```
...
**图表来源**
- [start_backend.py](file://start_backend.py#L11-L36)
...
**图表来源**
- [backend/pyproject.toml](file://backend/pyproject.toml#L6-L23)
...
- 一键启动优化
  - 全局启动脚本使用后台运行模式，避免阻塞终端。
- **进程管理优化**
  - **已改进**：后端启动脚本使用 Popen 实现更好的进程控制，支持优雅关闭。
...
- [start_all.sh](file://start_all.sh#L7-L11)
- [start_backend.py](file://start_backend.py#L11-L18)
...
- 全局启动脚本问题
  - 确认 bash shell 可用，检查脚本权限（chmod +x start_all.sh）
  - 如需单独启动，可使用 start_backend.py 和 start_frontend.py
- **后端进程管理问题**
  - **新增**：如果后端进程无法正常关闭，检查信号处理机制是否正常工作
  - **新增**：使用 Ctrl+C 或 kill -TERM 发送信号，验证优雅关闭流程
  - **新增**：确认 Popen 进程控制是否正常，检查进程 ID 和资源清理

**章节来源**
- [check_config.py](file://check_config.py#L55-L82)
- [backend/.env.example](file://backend/.env.example#L1-L14)
- [backend/app/main.py](file://backend/app/main.py#L64-L68)
- [frontend/vite.config.js](file://frontend/vite.config.js#L14-L19)
- [start_all.sh](file://start_all.sh#L1-L14)
- [start_backend.py](file://start_backend.py#L20-L36)

## 结论
通过本指南，您可以完成环境准备、配置与依赖安装、数据库初始化以及前后端的启动与基本使用。系统提供了资料上传、知识点提炼与题目生成的核心能力，适合个人学习与教学辅助场景。**新增的全局启动脚本进一步简化了开发环境的启动流程，提升了开发效率**。**后端启动脚本的重大改进实现了优雅关闭和进程管理，解决了开发和部署过程中的资源清理问题**。如需扩展，可在现有 API 与数据库模型基础上继续开发。
...
- 操作系统
  - 支持 Linux/macOS/Windows（需要 bash 环境）
...
- 一键启动（推荐）
  - 在项目根目录运行：bash start_all.sh
  - 脚本将自动启动后端（8000端口）和前端（5173端口）
- 分别启动
  - 后端：python start_backend.py（**已改进，支持优雅关闭**）
...
### 基本使用示例
- 启动后端与前端
  - 使用全局启动脚本一键启动：bash start_all.sh
  - 或分别运行启动脚本：python start_backend.py 和 python start_frontend.py
...
### 后端启动脚本重大改进说明
**新增**：后端启动脚本已实现重大改进，解决资源清理问题

- **进程控制改进**
  - 从 subprocess.run 改为 subprocess.Popen，实现对子进程的完全控制
  - 通过 process.terminate() 和 process.wait() 实现优雅关闭
  - 支持后台运行和前台调试两种模式
...
- **使用建议**
  - 在开发过程中使用 Ctrl+C 正常关闭后端服务
  - 在部署环境中使用 kill -TERM 发送优雅关闭信号
  - 避免强制杀死进程导致资源泄漏
