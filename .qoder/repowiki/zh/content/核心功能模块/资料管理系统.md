...
**更新** 系统现已扩展支持多格式文件上传和URL内容创建功能，为用户提供更灵活的内容导入方式。
...
- 资料API：上传资料、同步处理（知识点提取+题目生成）、SSE进度流、删除资料、**新增**文件上传和URL创建
...
**图表来源**
- [backend/app/api/materials.py](file://backend/app/api/materials.py#L32-L101)
...
**图表来源**
- [backend/app/api/materials.py](file://backend/app/api/materials.py#L176-L229)
...
#### 文件上传接口
- **/api/materials/upload-file**：支持PDF、DOCX、MD、TXT格式的文件上传
- 表单参数：title（标题）、direction_id（学习方向）、file（文件）
- 流程：文件上传 → 文本提取 → 同步处理（知识点提取+题目生成）

#### URL内容创建
- **/api/materials/from-url**：从网页URL创建学习资料
- 请求参数：title（标题）、url（网页地址）、direction_id（学习方向）
- 流程：URL抓取 → 文本提取 → 同步处理（知识点提取+题目生成）

```mermaid
flowchart TD
FileUpload["文件上传"] --> Validate["验证文件格式"]
Validate --> Extract["提取文本内容"]
URLCreate["URL创建"] --> Fetch["抓取网页内容"]
Fetch --> Extract
Extract --> Process["同步处理"]
Process --> Points["提取知识点"]
Points --> Questions["生成题目"]
Questions --> Save["保存到数据库"]
Save --> Complete["处理完成"]
```

**图表来源**
- [backend/app/api/materials.py](file://backend/app/api/materials.py#L116-L174)
...
- [backend/app/api/materials.py](file://backend/app/api/materials.py#L325-L334)
...
- 更新资料学习方向
  - 方法：PATCH
  - 路径：/api/materials/{material_id}
  - 请求体：UpdateMaterialDirectionRequest（包含direction_id）
  - 返回：MaterialResponse
...
- 更新解析任务学习方向
  - 方法：PATCH
  - 路径：/api/parse/tasks/{task_id}
  - 请求体：UpdateDirectionRequest（包含direction_id）
  - 返回：ParseTaskResponse
...
- **新增** 将解析任务转换为资料并生成题目
  ...
  - 返回：MaterialResponse
  - 条件：任务状态必须为COMPLETED且包含原始文本

**章节来源**
- [backend/app/api/materials.py](file://backend/app/api/materials.py#L104-L380)
- [backend/app/api/parse.py](file://backend/app/api/parse.py#L1-L177)
- [backend/app/schemas/schemas.py](file://backend/app/schemas/schemas.py#L36-L200)
...
- **新增** 文件大小限制：配置max_file_size控制上传文件大小，防止内存溢出
...
- **新增** 文件上传失败：检查文件格式、大小限制和服务器权限
- **新增** URL创建失败：验证URL有效性、网络连接和页面可访问性

**章节来源**
- [backend/app/api/materials.py](file://backend/app/api/materials.py#L138-L144)
- [backend/app/services/extractor_service.py](file://backend/app/services/extractor_service.py#L23-L37)
- [backend/app/services/extractor_service.py](file://backend/app/services/extractor_service.py#L93-L118)
- [backend/app/api/materials.py](file://backend/app/api/materials.py#L325-L334)

## 结论
本系统围绕"资料上传—AI解析—题目生成—实时反馈—状态管理"构建了完整的知识管理闭环。通过模块化的服务设计、清晰的API边界与完善的错误处理机制，既满足当前功能需求，也为后续扩展（如异步队列、缓存、监控）提供了良好基础。**新增的多格式文件上传和URL内容创建功能进一步提升了系统的灵活性和用户体验。**
...
- 单元测试：验证知识点提取与题目生成流程
- 端到端流程：模拟完整处理链路
- **新增** 文件上传测试：验证多格式文件解析功能
- **新增** URL创建测试：验证网页内容抓取功能
...
### 前端使用示例
- **文件上传**：使用FormData格式，支持PDF、DOCX、MD、TXT
- **URL创建**：发送JSON请求，包含标题、URL和学习方向
- **进度监听**：使用EventSource监听SSE流，实时显示处理进度

**章节来源**
- [frontend/src/api/index.js](file://frontend/src/api/index.js#L25-L36)
- [frontend/src/api/index.js](file://frontend/src/api/index.js#L67-L76)