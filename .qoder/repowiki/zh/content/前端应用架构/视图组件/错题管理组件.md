...
## 更新摘要
**变更内容**
- 新增易错题过滤功能：前端已实现按错误频率（error_prone）过滤的筛选器
- 错误频率字段：后端模型已包含 error_count 和 error_prone 字段
- 前端展示增强：支持显示错误次数和易错题标签
- 后端API待完善：当前后端API尚未支持 error_prone 参数过滤
...
## 简介
本文件面向"错题管理组件"，系统性阐述其在系统中的定位与职责：负责错题的采集、分类、展示、复习与统计分析。后端通过 API 提供错题列表、详情、更新与删除；前端提供筛选、排序、答案查看、标记掌握与复习记录等功能；测验模块在客观题答错时自动收录为错题；数据库模型定义了错题与其关联的题目、答题记录之间的关系。**新增功能**：支持按错误频率和掌握状态进行更精细的错题管理，包括易错题标识和错误次数统计。
...
- 错题视图：提供筛选（按方向、掌握状态、**易错题**）、答案查看、标记掌握、记录复习。
- 错题模型：与题目、答题记录建立外键关联，支持按方向过滤与按创建时间倒序，**新增**：包含错误次数和易错题标识字段。
- 测验集成：主观题由 AI 评分，客观题精确匹配，错误即自动创建错题记录。
- 数据库与模式：定义错题、题目、答题记录、学习方向等实体及枚举类型，**新增**：支持错误频率统计和易错题识别。
...
## 架构总览
错题管理贯穿"测验—错题—复习—统计"的闭环流程。测验提交时根据题型评分，错误自动写入错题表；前端错题页支持筛选与排序，用户可标记掌握、记录复习次数；后端提供错题 CRUD 接口与详情加载。**新增**：前端实现易错题过滤，支持按错误频率进行精细化管理。
...
U->>FE : 打开错题页
FE->>AX : GET /api/mistakes?direction_id=&mastered=&error_prone=
AX->>API : 发起请求
API->>DB : 查询错题并关联题目
DB-->>API : 返回错题列表
API-->>AX : 返回数据
...
- **更新**：当前列表接口不支持 error_prone 参数过滤，需要扩展以支持易错题筛选。
...
Patch --> SetFields["设置 mastered/review_count/error_prone"]
...
### 前端视图：错题列表与交互
- 筛选器：按学习方向与掌握状态筛选；**新增**：易错题筛选器，支持显示所有题目或仅显示易错题。
- 展示逻辑：每条错题卡片显示题目类型、难度星级、题目内容与选项；支持点击查看/隐藏正确答案与解析；展示复习次数与掌握标签；**新增**：显示错误次数和易错题标签。
- 操作按钮：查看答案、标记掌握/未掌握、记录复习（调用后端 PATCH 接口）。
- 加载与空态：加载中提示、空列表提示与引导文案。
- **新增**：前端过滤逻辑，根据 error_prone 状态在客户端进行筛选。
...
ErrorProne["易错题筛选"] --> FrontFilter["前端过滤<br/>error_prone === true"]
FrontFilter --> Display["显示易错题"]
```
...
- **新增**：包含 error_count（错误次数，默认1）和 error_prone（是否为易错题，默认False）字段。
...
- **新增**：错题创建时自动设置 error_count = 1，后续错误将递增此计数。
...
### 筛选与排序机制
- 筛选维度：按学习方向（通过题目-资料-方向链路）、按掌握状态（布尔值）、**新增**：按易错题状态（布尔值）。
- 排序规则：按创建时间倒序，优先展示最新错题。
- 前端实现：通过查询参数传递筛选条件，后端联表过滤并排序。
- **更新**：当前后端 API 不支持 error_prone 参数，前端实现为客户端过滤。
...
- **新增**：显示错误次数（出错 X 次）和易错题标签（红色标签），帮助用户快速识别高频错误题目。
...
- **新增**：基于 error_count 和 error_prone 字段，可实现更精细的统计分析，识别用户的薄弱知识点和常见错误模式。
...
- **新增**：支持按易错题状态进行批量操作，如批量标记为已掌握或批量删除。
...
- **新增**：利用 error_count 和 error_prone 字段，为用户提供个性化的学习建议，优先推荐易错题相关的知识点巩固练习。
...
ERRORPRONE["前端易错题过滤"] --> FEVUE["MistakesView.vue"]
```
...
- **新增**：易错题过滤在前端进行，避免了后端复杂查询，但可能影响大数据集的性能。
...
- **新增**：易错题筛选无效：检查前端过滤逻辑是否正确，确认 error_prone 参数是否正确传递给后端。
...
## 结论
错题管理组件已具备完整的采集、展示与基本复习功能，与测验模块无缝衔接，实现了"错误即收录"的自动化流程。**新增的易错题过滤功能**显著增强了错题管理的精细化程度，用户可以通过错误频率快速识别薄弱环节。建议后续增强统计分析、批量操作、导出与知识点关联，以进一步完善个性化学习路径与复习策略。同时需要完善后端API以支持 error_prone 参数过滤，提升系统的整体性能和用户体验。
...
- [frontend/src/api/index.js](file://frontend/src/api/index.js#L44-L49)
- [backend/app/models/models.py](file://backend/app/models/models.py#L155-L168)
- [frontend/src/views/MistakesView.vue](file://frontend/src/views/MistakesView.vue#L15-L18)
- [frontend/src/views/MistakesView.vue](file://frontend/src/views/MistakesView.vue#L98-L102)