...
## 更新摘要
**变更内容**
- 新增 question_ids 字段的详细说明，该字段作为 Exam 实体的重要组成部分，提供题目ID的持久化存储
- 更新测验创建流程，说明 question_ids 在创建过程中的作用和重要性
- 增强测验恢复和数据迁移场景的说明
- 更新测验详情获取逻辑，强调 question_ids 的优先使用策略
...
**新增**：测验实体现已包含 question_ids 字段，该字段提供题目ID的持久化存储，支持测验恢复和数据迁移场景，确保测验的稳定性和可恢复性。
...
graph TB
subgraph "后端"
M["models.py<br/>定义 Exam/枚举/关系<br/>包含 question_ids 字段"]
S["schemas.py<br/>定义 ExamCreate/ExamResponse/ExamResult 等"]
A["exams.py<br/>测验 API 实现<br/>使用 question_ids 进行题目持久化"]
end
subgraph "前端"
...
## 核心组件
- 测验实体（Exam）：持久化测验的基本信息与结果，包含 question_ids 字段用于题目ID持久化
...
**更新**：Exam 实体现在包含 question_ids 字段，该字段是测验功能的关键组成部分，确保题目选择的可追溯性和测验的完整性。
...
- **question_ids**：**新增** JSON 类型字段，存储选中的题目ID列表，用于测验的持久化和恢复
- created_at/completed_at：时间戳，用于统计与排序

**更新**：新增的 question_ids 字段是测验实体的核心改进，它确保了题目选择的可追溯性和测验的完整性。该字段在测验创建时被持久化，支持后续的测验恢复和数据迁移场景。
...
### Pydantic 模式（输入/输出）
- ExamCreate：创建测验时的请求体，包含 direction_id、mode、time_limit、score_type、question_count、material_ids
- ExamResponse：测验响应，包含上述字段与状态、时间戳
- ExamWithQuestions：创建后返回的完整测验详情，包含题目列表
- ExamResult：提交后返回的结果，包含总题数、正确数、最终分数与等级、答题明细

**更新**：ExamCreate 模式现在包含 material_ids 字段，允许用户指定特定资料ID列表，这与 question_ids 字段配合使用，提供了更灵活的题目选择机制。
...
**更新**：前端现在支持资料选择功能，用户可以选择特定的学习资料来生成测验，这与后端的题目优先级选择算法配合使用。
...
### 测验状态转换与完成后的成绩计算
- 创建阶段：根据方向与题量随机抽取题目，初始化 Exam 为 in_progress，并持久化 question_ids 字段
- 提交阶段：
...
```mermaid
flowchart TD
Start(["提交开始"]) --> LoadExam["加载测验与题目<br/>优先使用 question_ids"]
...
**更新**：新增了 question_ids 字段在创建和获取流程中的作用说明。
...
API --> MODELS["models.py<br/>包含 question_ids 字段"]
...
**更新**：图表中明确标注了 models.py 包含 question_ids 字段。
...
- **question_ids 优化**：通过持久化题目ID列表，避免重复查询题目数据，提高测验详情加载性能
- 建议
  - 对高频查询（如按方向/状态过滤）建立索引
  - 限制单次题量上限，避免超大提交包
  - 主观题评分异步化，提升用户体验
  - **利用 question_ids 字段进行缓存和预加载**

**更新**：新增了 question_ids 字段对性能的影响分析。
...
- **question_ids 相关问题**
  - 现象：测验详情加载异常或题目顺序错误
  - 排查：检查 question_ids 字段是否正确持久化，确认题目ID列表的完整性
  - 参考：[测验详情获取逻辑](file://backend/app/api/exams.py#L226-L239)

**更新**：新增了 question_ids 相关的故障排查指南。
...
## 结论
测验实体（Exam）通过清晰的字段与枚举约束，配合前后端协作，实现了从创建、答题到评分与结果展示的完整闭环。timed/untimed 模式满足不同场景需求；hundred/grade 评分方式兼顾直观与等级化；完善的错误处理与状态机保障了流程稳定性。

**新增**：question_ids 字段的引入是本次更新的重要改进，它提供了测验的持久化能力，确保了题目选择的可追溯性和测验的完整性。这一改进支持测验恢复、数据迁移和性能优化等多种场景，显著提升了系统的稳定性和用户体验。

建议在生产环境中进一步完善索引、异步评分与限流策略，持续优化用户体验与系统性能。
...
**更新**：新增了 question_ids 字段的详细说明和使用场景。

**章节来源**
- [models.py](file://backend/app/models/models.py#L122-L134)
